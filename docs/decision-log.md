...
**Open decisions:** D25, D26, D28, D30, D32, raised by [roadmap.md](roadmap.md) and listed in its decision calendar with the step that needs each; D31 is deferred to the go/no-go. D34 (an operator editor in the prototype) is decided. D35 (prototype front door: Hebrew default + a track picker instead of location codes), D36 (explicit start and end points), and D37 (GPS arrival pulled forward, revising D20) are decided. D27 (hosting), D29 (build method), and D33 (platform scope) are decided. D30 (a Google Play developer account, for testers' installs and updates) is with the owner. **Provisional (REVISIT after the prototype is played):** D14, D18.
...
**RTL follow-up (owner chose "force full RTL"):** the native layout direction now tracks the UI language via `I18nManager` (Hebrew → RTL, English → LTR), applied on the next relaunch. **Verified 2026-08-22 on the release APK on the Android emulator (en-US):** the home opens in Hebrew and lists the Spring Trail fetched from the hosted API; tapping it opens the track; the layout mirrors to RTL — on the en-US emulator the first launch is a left-to-right shell and the next launch is fully RTL (matching the Settings restart note), and a Hebrew-locale device is RTL from the first launch. A new release APK (`apps/mobile/android/app/build/outputs/apk/release/app-release.apk`) was built with these changes.

### D36. Explicit start and end points — DECIDED 2026-08-26
**Customer (via the owner, 2026-08-26):** "every track must have obvious starting and ending points; these can be the same point, or different points."
**Options:** (a) explicit, optional start and end points on the track — a location plus a short note, shown as "Start here" / "The trail ends here", the first/last station serving when unset; (b) no new fields — present the first station as the start and the last as the end.
**Decision (owner):** (a).
**Shape:** per-leg optional `start` and `end` waypoints `{ location, note }` (the track's meeting point is the first leg's `start`, its finish the last leg's `end`). The `note` is a one-liner ("next to the kiosk"), optional, but complete in every language when present; the leg's `outro` stays the closing text and is now actually shown on the finish screen. The two may coincide for a circular route — "same as start" is a console convenience, not a schema flag. Waypoints are informational: no events, no arrival, no scoring. On a tiles map a waypoint needs a location inside the leg's map region (the derived region now includes waypoints); an image map would need an image position. No schema-version bump (optional additions).
**Where it shows:** a "Meeting point" card on the track details screen before download; distinct Start/Finish markers on the play map; "Start here: …" with the first clue; "The trail ends here" on the result screen. The console edits both above and below the station list.

### D37. GPS arrival verification, pulled forward — DECIDED 2026-08-26
**Customer (via the owner):** "make sure we can actually use the GPS to verify the user arrived at the location; this also requires the app to request permissions." This revises D20, which kept the prototype to manual arrival.
**What already existed:** the content model (`arrival.methods` may include `gps`, `radiusMeters` 10–500, default 30), the builder invariant (GPS needs a location), and the app's play screen, which offers "You've reached {station}" when the party is within the radius with a usable fix (accuracy ≤ max(radius, 50 m)) — dormant only because no station enabled it and the console had no control.
**Decision (owner):** pull it forward.
**Shape:** the console gets a per-station "Verify arrival by GPS" toggle and radius (off for new stations; the radius is drawn on the station map). The app asks for location with a plain rationale, foreground only (§5.7); shows the distance to the current station; within the radius it *offers* the confirmation (§4.3 — an offer, not an interruption); the manual "We're here" check-in is always available (D6, D11), with one line explaining why it might be needed (location off, no fix after 30 s, poor accuracy). The engine now rejects an arrival method the station does not offer.
**Deferred (owner decisions, not made here):** recording the party's position on arrival events (a data-collection choice; design §5.7 promises no background tracking); auto-opening a station on arrival; making GPS mandatory by hiding manual check-in.
